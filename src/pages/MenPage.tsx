...
export function MenPage() {
  ...
  const menProducts = state.products.filter(product =>
    product.category_names?.includes('Men') ||
    product.tags.some(tag => tag.toLowerCase().includes('men'))
  );

  const filteredProducts = useProductFiltering(menProducts, state.filters, state.sortBy);
  ...
  return (
    <main className="container mx-auto px-4 py-8">
      <div className="mb-8">
        ...
        <p className="text-gray-600">Explore fashion, accessories, and essentials for men.</p>
      </div>

      <div className="flex gap-8">
        <FilterSidebar isOpen={isFilterOpen} onToggle={() => setIsFilterOpen(!isFilterOpen)} categoryFilter="Men" />
